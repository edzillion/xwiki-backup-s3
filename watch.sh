...
function restore {
  ...
  if ! aws s3 sync "$REMOTE" "$LOCAL"; then
    ...
    if [[ -z `ls -A $LOCAL` ]]; then
      ...
      if ! ./restore.sh; then
        error_exit "xwiki restore failed"
      else
        echo "xwiki restore completed"
        return 0
      fi
    fi
  fi  
...
function backup {
  echo "backup xwiki"
  if [ -e "/data/data/xwiki.cfg" ]; then
    echo "xwiki located, attempting xwiki backup"  
    if ./backup.sh; then
    ...
    echo "can't find xwiki, no backup"
    return 1
  fi
}

function final_backup {
  echo "final backup xwiki"
  if [ -e "/data/data/xwiki.cfg" ]; then
    echo "xwiki located, attempting final xwiki backup"
    if ./backup.sh; then
      echo "s3 backup $LOCAL => $REMOTE"
    ...
    echo "can't find xwiki, no final backup"
    exit 1
  fi
}
...
echo "xwiki-backup-s3 initialising..."
